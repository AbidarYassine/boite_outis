...
public class EditRoleController implements Initializable {
    ...
    @FXML
    private JFXComboBox<String> cr_role_name;
    @FXML
    ...
    @FXML
    private CustomTextField requette_er_role;
    ...
    public void editRole(ActionEvent actionEvent) {
        if (validationInput()) {
            ...
            role.setRole_name(" "+cr_role_name.getValue()+" ");
            ...
            try {
                String result = roleDao.editRoleDao(role);
                if (result != null) {
                    requette_er_role.setText(result);
                    AlertUtil.ShowAlert("Congratulation!!", "Votre role a bie ete modifiée", Alert.AlertType.INFORMATION);
                }
            } catch (Exception e) {
    ...
    private void loadDataIntoChoiceBox() throws SQLException {
        ...
        cr_role_name.getItems().addAll(observableList);
    ...
    private boolean validationInput() {
        if (cr_role_name.getSelectionModel().isEmpty() || cr_role_identified.getSelectionModel().isEmpty()) {
